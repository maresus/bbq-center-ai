(function() {
  ...
  const CONFIG = {
    ...
    welcomeMessage: 'Pozdravljeni! 🔥 Sem AI pomočnik BBQ Center Ptuj — Weberovega pooblaščenega prodajalca žarov in BBQ opreme. Pomagam pri izbiri pravega žara, odgovarjam o brandih, cenah in dostavi. Kako vam lahko pomagam?',
  ...
  const styles = `
    ...
    #mo-inquiry-btn:hover { background: #c53509; }

    #mo-inquiry-form-view {
      display: none;
      flex-direction: column;
      flex: 1;
      overflow: hidden;
    ...
    #mo-inquiry-form-view.kv-visible {
      display: flex;
    }

    #mo-inquiry-form-header {
      background: ${CONFIG.accentColor};
      color: #fff;
      padding: 12px 16px;
      display: flex;
    ...
    #mo-inquiry-submit:hover { background: #c53509; }
    ...
    #mo-inquiry-success .kv-success-icon {
      width: 56px;
      height: 56px;
      background: ${CONFIG.accentColor};
      border-radius: 50%;
    ...
    #mo-inquiry-success .kv-success-icon svg {
      width: 28px;
      height: 28px;
      fill: white;
    }

    #mo-inquiry-success h4 {
      color: ${CONFIG.accentColor};
      font-size: 16px;
      margin: 0 0 8px;
    }

    #mo-inquiry-success p {
      color: #666;
      font-size: 13px;
      line-height: 1.5;
      margin: 0;
  ...
  let sessionId = localStorage.getItem('bc_widget_session') || generateSessionId();
  localStorage.setItem('bc_widget_session', sessionId);

  let storedMessages = [];
  try {
    const stored = localStorage.getItem('bc_widget_messages');
  ...
  function saveMessages() {
    ...
    localStorage.setItem('bc_widget_messages', JSON.stringify(toSave));
  }

  function clearConversation() {
    storedMessages = [];
    localStorage.removeItem('bc_widget_messages');
  ...
  function createWidget() {
    ...
    ['Na žaru mojster? 🔥', 'Kateri žar izbrati?', 'Weber Academy tečaji'].forEach(function(text) {
      var btn = document.createElement('button');
      btn.setAttribute('style', cardStyle);
      btn.textContent = text;
      btn.onclick = function(e) { e.stopPropagation(); e.preventDefault(); hideCards(); openPanel(); setTimeout(function() { document.getElementById('kv-widget-input').value = text; sendMessage(); }, 200); };
    ...
    panel.innerHTML = `
      <div id="kv-widget-header">
        <div id="kv-widget-header-icon"><img src="${CONFIG.logoUrl}" alt="BBQ Center" style="width:48px;height:48px;object-fit:contain;border-radius:8px;background:white;padding:4px;" onerror="this.style.display='none'"></div>
      ...
      <div id="kv-chat-view" style="display:flex;flex-direction:column;flex:1;overflow:hidden;min-height:0;">
        ...
        <div id="kv-widget-disclaimer">🤖 Ta asistent je umetna inteligenca (AI) — EU AI Act čl. 50. Odgovori so informativne narave. Za nakup pokličite <a href="tel:051404700">051 404 700</a>.</div>
      ...
      <div id="mo-inquiry-form-view">
        <div id="mo-inquiry-form-header">
          <span>Povpraševanje za BBQ Center</span>
          <button id="mo-inquiry-back">← Nazaj</button>
        </div>
        <div id="mo-inquiry-form-scroll">
          ...
          <div class="kv-field">
            ...
            <div class="kv-checkboxes">
              <label class="kv-checkbox-item"><input type="checkbox" value="Weber žar"> Weber žar</label>
              <label class="kv-checkbox-item"><input type="checkbox" value="Pribor in oprema"> Pribor in oprema</label>
              <label class="kv-checkbox-item"><input type="checkbox" value="Noži"> Noži</label>
              <label class="kv-checkbox-item"><input type="checkbox" value="Weber Academy"> Weber Academy</label>
              <label class="kv-checkbox-item"><input type="checkbox" value="Dostava / naročilo"> Dostava / naročilo</label>
              <label class="kv-checkbox-item"><input type="checkbox" value="Reklamacija"> Reklamacija</label>
            </div>
          </div>
          <div class="kv-field">
            <label>Sporočilo</label>
            <textarea id="jf-sporocilo" placeholder="Model žara, dimenzije, posebne zahteve..."></textarea>
          </div>
        ...
        <div id="mo-inquiry-success">
          ...
          <h4>Povpraševanje uspešno poslano!</h4>
  ...
  function openPanel() {
    ...
    localStorage.setItem('bc_widget_open', 'true');
  ...
  function closePanel() {
    ...
    localStorage.setItem('bc_widget_open', 'false');
    showCards();
  }
  ...
  function renderMarkdown(text) {
    ...
    escaped = escaped.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener" style="color:#e8420a;text-decoration:underline;">$1</a>');

    escaped = escaped.replace(/(?<!=["'])(https?:\/\/[^\s<>"')\]]+)/g,
      '<a href="$1" target="_blank" rel="noopener" style="color:#e8420a;text-decoration:underline;">$1</a>');

    escaped = escaped.replace(/(?<![/"'=])(www\.[a-zA-Z0-9][^\s<>"')\]]+)/g,
      '<a href="https://$1" target="_blank" rel="noopener" style="color:#e8420a;text-decoration:underline;">$1</a>');
  ...
  async function sendMessage() {
    ...
    try {
      ...
      hideTyping();
      addMessage('Oprostite, trenutno ni mogoče vzpostaviti povezave. Pokličite nas: 051 404 700.', 'bot');
      console.error('[BBQ Widget] Error:', err);
    }
  ...
  async function submitInquiryForm() {
    ...
    try {
      var res = await fetch(CONFIG.inquiryUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert('Napaka pri pošiljanju. Pokličite: 051 404 700');
      btn.disabled = false;
