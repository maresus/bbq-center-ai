...
def _send_email(to: str, subject: str, html: str) -> bool:
    if not SMTP_HOST or not SMTP_USER or not to:
        print(f"[email] SMTP ni konfiguriran ali ni prejemnika — preskočeno")
        return False
    try:
...
async def submit_inquiry(payload: InquiryRequest):
    ...
    <div style="background:#e8420a;padding:24px;border-radius:8px 8px 0 0;">
      <h2 style="color:#fff;margin:0;">Novo povpraševanje — BBQ Center</h2>
    ...
    <div style="background:#f8f9fa;padding:24px;border:1px solid #e0e0e0;border-radius:0 0 8px 8px;">
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr><td style="padding:8px 0;color:#666;"><b>Telefon</b></td><td><a href="tel:{payload.telefon}" style="color:#e8420a;">{payload.telefon}</a></td></tr>
        <tr><td style="padding:8px 0;color:#666;"><b>Email</b></td><td><a href="mailto:{payload.email}" style="color:#e8420a;">{payload.email}</a></td></tr>
    ...
    _send_email(ADMIN_EMAIL, f"Povpraševanje BBQ Center: {payload.ime}", admin_html)

    confirm_html = f"""
    <html><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
    <div style="background:#e8420a;padding:24px;border-radius:8px 8px 0 0;">
      <h2 style="color:#fff;margin:0;">Hvala za povpraševanje, {payload.ime}!</h2>
      <p style="color:rgba(255,255,255,0.9);margin:4px 0 0;">BBQ Center Ptuj — Weber pooblaščeni prodajalec</p>
    </div>
    <div style="background:#f8f9fa;padding:24px;border:1px solid #e0e0e0;border-radius:0 0 8px 8px;">
      <p>Prejeli smo vaše povpraševanje. Naša ekipa vas bo kontaktirala v najkrajšem možnem času.</p>
      <div style="background:#1c1c1c;color:#fff;padding:16px;border-radius:8px;margin:20px 0;">
        <b>Naši kontakti:</b><br>
        📞 051 404 700<br>
        ✉ info@bbq-center.si<br>
        📦 Naročila: podpora@bbq-center.si<br>
        🎓 Weber Academy: eventi@bbq-center.si<br>
        📍 Ormoška cesta 28c, 2250 Ptuj
      </div>
      <p style="color:#999;font-size:12px;">BBQ Center Ptuj · Ormoška cesta 28c, 2250 Ptuj</p>
    </div>
    </body></html>"""
    _send_email(payload.email, "Potrditev povpraševanja — BBQ Center Ptuj", confirm_html)
    return {"ok": True}
